...
<div class="content">
	...
	<div class="card">
		...
		<div class="card-body">
			<div id="info"></div>
			<form id="form" method="post" data-route="{{ url('crew/update-rombel/'.$idrombel) }}">
				{{ csrf_field() }}
				<div class="row">
					<div class="col-md-8">
						<fieldset>
							<div class="form-group row">
								...
								<div class="col-lg-9">
									...
									@foreach ($getSekolah as $skl)
										<option {{selectAktif($getRombel->rblSklId,$skl->sklId)}} value="{{ encrypt_url($skl->sklId) }}">{{ $skl->sklNama}}</option>
									@endforeach
									</select>
								</div>
							</div>
							<div class="form-group row">
								<label class="col-lg-3 col-form-label">Level Kelas</label>
								<div class="col-lg-9">
									<select required data-placeholder="Pilih Level Kelas"  name="kls" id="kls"  class="form-control select-fixed-single" data-fouc>
									<option></option>
									@foreach ($getKelas as $kls)
										<option {{selectAktif($getRombel->rblKlsId,$kls->klsId)}} value="{{ encrypt_url($kls->klsId) }}">{{ $kls->klsNama}}</option>
									@endforeach
									</select>
								</div>
							</div>
							<div class="form-group row">
								...
								<div class="col-lg-9">
									...
									@foreach ($getJurusan as $jrs)
										<option {{selectAktif($getRombel->rblJrsId,$jrs->jrsId)}} value="{{ encrypt_url($jrs->jrsId) }}">{{ $jrs->jrsNama}}</option>
									@endforeach
									</select>
								</div>
							</div>
							<div class="form-group row">
								...
								<div class="col-lg-9">
									<input value="{{$getRombel->rblKode}}" required type="text" name="rblKode" class="form-control" placeholder="Misal 2021AKL1 atau 1920TBSM1">
							...
							<div class="form-group row">
								...
								<div class="col-lg-9">
									<input value="{{$getRombel->rblNama}}" required type="text" name="namarombel" class="form-control" placeholder="Misal AKL1 atau TBSM2">
								</div>
							</div>
