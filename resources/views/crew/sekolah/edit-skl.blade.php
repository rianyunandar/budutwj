					...
					<div class="col-md-8">
						<fieldset>
							<div class="form-group row">
								...
								<div class="col-lg-9">
									...
									@foreach ($tingkat_pendidikan as $tk)
										<option {{ selectAktif($getSkl->sklTkpdId,$tk->tkpdId) }} value="{{ encrypt_url($tk->tkpdId)}}">{{ $tk->tkpdNama.' '.$tk->tkpdStatus}}</option>
									@endforeach
									</select>
								</div>
							</div>
							<div class="form-group row">
								...
								<div class="col-lg-9">
									...
									@foreach ($yayasan as $yas)
										<option {{ selectAktif($getSkl->sklMsysId,$yas->msysId) }} value="{{ encrypt_url($yas->msysId)}}">{{ $yas->msysNama}}</option>
									@endforeach
									</select>
								</div>
							</div>
							...
							<div class="form-group row">
								...
								<div class="col-lg-9">
									<input value="{{ $getSkl->sklNis }}" type="text" name="nis_skl" class="form-control" placeholder="NIS Sekolah" >
								</div>
							</div>
							<div class="form-group row">
								<label class="col-lg-3 col-form-label">ID Dapodik Sekolah</label>
								<div class="col-lg-9">
									<input value="{{ $getSkl->sklIdDapodik }}" type="text" name="dapo_skl" class="form-control" placeholder="ID Dapodik Sekolah" >
								</div>
							</div>
							<div class="form-group row">
							...
							<div class="form-group row">
								...
								<div class="col-lg-9">
									<input value="{{ $getSkl->sklEmail }}"  type="email" name="email_skl" class="form-control" placeholder="Email Sekolah" >
								</div>
							</div>
							<div class="form-group row">
