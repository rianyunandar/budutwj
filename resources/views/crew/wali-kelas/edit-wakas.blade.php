	...
	<div class="card">
		...
		<div class="card-body">
			...
			<div class="row">
				...
				<hr class="mt-1 mb-1"/>
				<form id="update" data-route="{{ url('crew/update-wakas/'.$idwakas) }}">
				{{ csrf_field() }}
				<div class="row">
					<div class="col-md-8">
						<fieldset>
							<div class="form-group row">
								...
								<div class="col-lg-9">
									...
									@foreach ($getSekolah as $skl)
										<option {{ selectAktif($datawakas->wakasSklId,$skl->sklId) }} value="{{ encrypt_url($skl->sklId) }}">{{$skl->sklNama}}</option>
									@endforeach
									</select>
								</div>
							</div>
							<div class="form-group row">
								...
								<div class="col-lg-9">
									...
									@foreach ($getJurusan as $jrs)
										<option {{ selectAktif($datawakas->wakasJrsId,$jrs->jrsId) }} value="{{ encrypt_url($jrs->jrsId) }}">{{$jrs->jrsNama}}</option>
									@endforeach
									</select>
								</div>
							</div>
							<div class="form-group row">
								...
								<div class="col-lg-9">
									...
									@foreach ($getGuru as $guru)
										<option {{ selectAktif($datawakas->wakasUgrId,$guru->ugrId) }} value="{{ encrypt_url($guru->ugrId) }}">{{$guru->ugrFirstName.' '.$guru->ugrLastName}}</option>
									@endforeach
									</select>
								</div>
							</div>
							<div class="form-group row">
								...
								<div class="col-lg-6">
									...
									@foreach ($getRombel as $rbl)
										<option {{ selectAktif($datawakas->wakasRblId,$rbl->rblId) }} value="{{ encrypt_url($rbl->rblId) }}">{{$rbl->master_kelas->klsKode.' '.$rbl->rblNama}}</option>
									@endforeach
									</select>
								</div>
							</div>
