...
<!-- Content area -->
<?php 
// ini  page total absensi semua mapel yang sama pada hari berbeda
...
if(!empty($_GET['skl'])){ 
	$url = 'guru/json-tota-all-absen-mapel?skl='.$getskl.'&rbl='.$getrbl.'&bln='.$getbln.'&mpl='.$getmapel; 
} else{ $url='guru/json-tota-all-absen-mapel';}
					...
					<div class="form-group row">
						...
						<div class="col-lg-4">
							<select required data-placeholder="Pilih Mapel"  name="mapel" id="mapel"  class="form-control select-search" data-fouc>
								...
								<option value="{{ encrypt_url($data->majdId) }}">{{ $data->majdNama }} </option>
							@endforeach
							</select>
						</div>
