		...
		<div class="card-body">
			...
			<div class="card card-body">
				<div class="media align-items-center align-items-md-start flex-column flex-md-row">
					...
					<div class="media-body text-center text-md-left">
						<center>
						<h6 class="media-title font-weight-semibold">{{ $getData->tugasJudul }}</h6>
						{{ $getData->tugasMapelNama }} {{ empty($getData->tugasKi) ? ' ' : ' | KI '.$getData->tugasKi }} {{ empty($getData->tugasKd) ? ' ' : ' | KD '.$getData->tugasKd }}
						</center>
					</div>
			...
			<div class="row pb-3">
				<div class="col-md-12">
					<div class="row">
						<div class="col-md-12">
							<label><b>Daftar Rombel yang dapat mengakses Tugas </b></label>
						</div>
					</div>
					<div class="row">
						<div class="col-md-12">
							<div class="table-responsive">
								<table class="table table-bordered table-hover table-xs">
									...
									<tbody>
										<?php $no=1; ?>
										@foreach ($getDataRombel as $data)
										<tr>
											<td>{{ $no++ }}</td>
											<td><b>{{ $data->master_rombel->master_kelas->klsKode.' '.$data->master_rombel->rblNama }}</b></td>
											<td>{{ date('d-m-Y H:i:s', strtotime($data->tgsarTerbit))   }}</td>
											<td>{{ hariIndo(date('l', strtotime($data->tgsarTerbit)))   }}</td>
											<td>{{ date('d-m-Y H:i:s', strtotime($data->tgsarBatasTerbit))   }}</td>
											<td>{{ hariIndo(date('l', strtotime($data->tgsarBatasTerbit)))   }}</td>
											
											<td>
												@if($data->tgsarTampilSiswa == 1)
												...
												<div class="list-icons">
													<div class="dropdown">
														...
														<div class="dropdown-menu dropdown-menu-right">
															<button 
															data-tgl="{{ date('d-m-Y H:i:s', strtotime($data->tgsarTerbit))   }}"
															data-akhir="{{ date('d-m-Y H:i:s', strtotime($data->tgsarBatasTerbit))   }}"
															data-id="{{ $data->tgsarId }}"
															data-tampil="{{ $data->tgsarTampilSiswa }}"
															class="dropdown-item edit" data-toggle="modal" data-target="#modal_iconified"><i class="icon-pencil7"></i> Edit</button>
															<a id="delete" data-id="{{ $data->tgsarId }}"  class="dropdown-item"><i class="icon-trash"></i> Hapus</a>
														</div>
													</div>
												</div>
					...
					<div class="modal-dialog">
						<div class="modal-content">
							<div class="modal-header">
								<h5 class="modal-title"><i class="icon-menu7 mr-2"></i> &nbsp; <b>Edit Rombel Akses Tugas</b></h5>
								...
								<div class="form-group">
									<div class="row">
										...
										<div class="col-sm-6 pb-2">
											...
											<input value="" id="eakhir" type="text" placeholder="Masukan Tanggal" class="form-control timer">
										</div>
			...
			<div class="alert alert-info alert-styled-left alert-arrow-left bg-white">
				...
				Menu di Bawah ini bisa di gunakan untuk menambah rombel yang bisa mengakses Tugas<br>
				Pilih kelas untuk menampilkan Rombel, Pilih rombel yang belum ada atau belum di beri akses
				</div>
				<!-- /info alert -->
			<div class="row">
				<div class="col-md-12">
				<form data-route="{{ route('update.tugas.rombel.guru') }}" id="insert" enctype="multipart/form-data" >
					@csrf
					
					<input type="hidden" value="{{ encrypt_url($getData->tugasId) }}" name="idmateri">
					<input type="hidden" value="{{ encrypt_url($getData->tugasKode) }}" name="kodemateri">
					<input type="hidden" value="{{ encrypt_url($getData->tugasMapelKode) }}" name="kodemapel">
					<input type="hidden" value="{{ encrypt_url($getData->tugasSklId) }}" name="idskl">
...
<script type="text/javascript">
	...
	$(document).on('click','#editrblmateri',function(e){
		var id = $("#eid").val();
		var tgl = $("#eterbit").val();
		var akhir = $("#eakhir").val();
		var tampil = $("#etampil").val();
		var token = $("meta[name='csrf-token']").attr("content");
		$.ajax({
					type:'put',
					url:'{{ route('edit.data.tugas.rombel.guru') }}',
					data: { _token: token,id: id,tgl:tgl,tampil:tampil,akhir:akhir },
					beforeSend: function() {
						$("#pesanku").text("Proses....");
						$('.loader').show();
      		},
	...
	$(document).on('click','#delete',function(e){
		...
		var notyConfirm = new Noty({
			...
			Noty.button('Hapus <i class="icon-trash ml-2"></i>', 'btn bg-danger ml-1', function () {
				...
				$.ajax({
					type:'put',
					url:'{{ route('delete.tugas.rombel.guru') }}',
